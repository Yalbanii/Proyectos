...
public class PointsBatchConfig {
    ...
    public ItemWriter<AsistenciaPointsData> pointsUpdaterWriter() {
        return items -> {
            for (AsistenciaPointsData data : items) {
                ...
                try {
                    // Transacción MySQL
                    updatePassInMySQL(data);

                    // Si MySQL OK, actualizar MongoDB
                    updateAsistenciaInMongoDB(data);

                    log.info("✅ Asistencia procesada exitosamente: Pass={}, Puntos={}",
                            data.getPassId(), data.getPointsAwarded());

                } catch (Exception e) {
                    log.error("❌ Error procesando asistencia: {}", data, e);

                    // ✅ IMPORTANTE: Re-lanzar para que Spring Batch lo detecte
                    throw new RuntimeException(
                            "Failed to process assistance: " + data.getAsistenciaId(), e);
                }
            }
        };
    }
    ...
    @Transactional
    private void updatePassInMySQL(AsistenciaPointsData data) {
    ...
    @Transactional
    private void updateAsistenciaInMongoDB(AsistenciaPointsData data) {
